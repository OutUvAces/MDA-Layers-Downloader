...
# Test the improved boundary detection approach
...
# Check first few and last one
for i in [0, 1, len(coord_sets)-1]:
    if i < len(coord_sets):
        coord_set = coord_sets[i]
        print(f"\nSet {i}: {coord_set[0]}")
        if len(coord_set) >= 3:
            feature_text = coord_set[2]
            print(f"Feature text: {repr(feature_text[:80])}...")
            if i == len(coord_sets)-1:  # Last one
                print(f"Length: {len(feature_text)}")

print("\nTesting feature description extraction:")
