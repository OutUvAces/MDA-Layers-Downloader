...
# Test the Davis Strait warning with contact info
...
print("Testing Davis Strait warning with contact info:")
print("=" * 60)
print(davis_warning)
print("=" * 60)
...
print("\nExtracted coordinate sets:")
for i, coord_set in enumerate(coord_sets[:3]):  # Just show first few
    print(f"Set {i}: {coord_set[0]}")
    if len(coord_set) >= 3:
        feature_text = coord_set[2]
        print(f"  Feature text: {repr(feature_text[:80])}...")
print("...")

# Check the last coordinate set
if coord_sets:
    last_set = coord_sets[-1]
    print(f"\nLast set ({len(coord_sets)-1}): {last_set[0]}")
    if len(last_set) >= 3:
        feature_text = last_set[2]
        print(f"  Feature text length: {len(feature_text)}")
        print(f"  Feature text end: {repr(feature_text[-100:])}")

from downloaders.navigation_warnings import extract_feature_description

if coord_sets:
    print("\nTesting feature A description:")
